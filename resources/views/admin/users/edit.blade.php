                    ...
                    <form action="{{ route('users.update', $user->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-control @error('status') is-invalid @enderror">
                                <option value="aktif" {{ old('status', optional($user->identitas)->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="tidak" {{ old('status', optional($user->identitas)->status) == 'tidak' ? 'selected' : '' }}>Tidak</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="alasan" class="form-control @error('alasan') is-invalid @enderror">
                                <option value="">-</option>
                                <option value="sekolah di luar kota" {{ old('alasan', optional($user->identitas)->alasan) == 'sekolah di luar kota' ? 'selected' : '' }}>Sekolah
                                    di luar kota</option>
                                <option value="bekerja di luar kota" {{ old('alasan', optional($user->identitas)->alasan) == 'bekerja di luar kota' ? 'selected' : '' }}>Bekerja
                                    di luar kota</option>
                            </select>
                            @error('alasan')
                                <div class="invalid-feedback">{{ $message }}</div>
